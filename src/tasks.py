...
class Task(Entity):
    ...
    def abort_task(self, id: uuid.UUID, spec: dict):
        """Aborts a Task execution in the database upon spec.

        The spec is a field in the broader JSON that describes the Task
        and is provided during the creation of the Task. Upon this spec the
        abort request is treated.

        Args:
            id: The ID of the Task
            spec: The JSON describing the abort request.
        """
        ...
        with transaction():
            ...
            # Set the state of the task to 'failed'
            # self.set_exec_state(id, "failed")
            # if spec.get("status"):

            # Get the task, check that it is unterminated and that
            # we have authority to terminate it.
            task = sql_utils.task_execution_read(id)

            curuser = kutils.current_user()
            creator = task["creator"]
            if (not curuser.get("is_admin", False)) and curuser.get(
                "username"
            ) != creator:
                raise AuthorizationError(
                    f"Not authorized. Only the creator or an admin can abort a task.",
                    current_user=curuser.get("username"),
                    task_id=id,
                )

            # The user is an admin or the creator of the task, so we can proceed
            if task["exec_state"] not in ["running", "created"]:
                raise ConflictError(
                    f"Task '{id}' is terminated and no further updates are allowed.",
                    task_id=id,
                    task_state=task["exec_state"],
                )

            if spec.get("status") not in ["aborted", "failed"]:
                raise DataError(
                    f"Invalid status '{spec.get('status')}' provided for task termination."
                )

            # Get the job id if it exists
            job_id = task["tags"].get("job_id", None)
            self.save_termination_data(id, task, spec)

        # Here we are, we let the transaction commit, before we proceed to
        # abort the kubernetes job (if applicable!). This is because the
        # kubernetes call may take some time and we do not want to hold
        # the transaction open for too long, as it may lead to problems.
        if job_id is not None:
            # We have a job id, so we can proceed to abort the job
            engine = execution.exec_engine()
            try:
                engine.abort_task(job_id)
            except ApiException as e:
                raise ConflictError(
                    f"Task {id} aborted, but failed to cancel job '{job_id}'.",
                    message=str(e),
                    task_id=id,
                    job_id=job_id,
                )
    ...
    def save_termination_data(self, id: uuid.UUID, task: dict, spec: dict):
        """Saves the termination data of a Task execution in the database.

        The termination data is provided by the task runtime at the end of the execution,
        or by the task creator at abort time.
        """
