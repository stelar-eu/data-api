...
def api_user_create(json_data):
    ...
    if request.data:
        ...
        response = {
            ...
            "error": {
                ...
                "name": [
                    "No specifications provided to create a new user in the Data Catalog. Please specify at least a username, a password and email."
                ],
            },
        }
        return jsonify(response)
    ...
    response = requests.post(
        config["CKAN_API"] + "user_create", json=user_metadata, headers=package_headers
...
def api_user_update(json_data):
    ...
    if request.data:
        ...
        response = {
            ...
            "error": {
                "__type": "No specifications",
                "name": [
                    "No specifications provided for updating an existing user in the Data Catalog. Please specify the identifier of the user and values for the properties you wish to update."
                ],
            },
        }
        return jsonify(response)
    ...
    # Make a POST request to the CKAN API with the parameters
    response = requests.post(
        config["CKAN_API"] + "user_patch", json=user_metadata, headers=package_headers
    )  # auth=HTTPBasicAuth(config.username, config.password))
...
@app.input(
    schema.Identifier,
    location="json",
    example={"id": "02568a6c-9970-4650-87d7-26d4f7d64fd6"},
)
@app.output(schema.ResponseOK, status_code=200)
@app.doc(tags=["User Management"])
@app.auth_required(auth)
def api_user_delete(json_data):
    ...
    if request.data:
        ...
        response = {
            ...
            "error": {
                "__type": "No specifications",
                "name": [
                    "No specifications provided for deleting an existing user in the Data Catalog. Please specify the identifier of this user."
                ],
            },
        }
        return jsonify(response)
    ...
    # Make a POST request to the CKAN API with the parameters
    response = requests.post(
        config["CKAN_API"] + "user_delete", json=user_metadata, headers=package_headers
    )  # auth=HTTPBasicAuth(config.username, config.password))
...
def api_user_role(json_data):
    ...
    if request.data:
        ...
        response = {
            ...
            "error": {
                "__type": "No specifications",
                "name": [
                    "No specifications provided to assign role for a user in the Data Catalog. Please specify the id of the organization, the user id, and the role to be assigned (member/editor/admin)."
                ],
            },
        }
        return jsonify(response)
    ...
    # Make a POST request to the CKAN API with the parameters
    response = requests.post(
        config["CKAN_API"] + "organization_member_create",
...
@app.input(
    schema.NewToken,
    location="json",
    example={"user": "test_user5", "name": "test5_API_token"},
)
@app.output(schema.ResponseOK, status_code=200)
@app.doc(tags=["User Management"])
@app.auth_required(auth)
def api_token_create(json_data):
    ...
    if request.data:
        ...
        response = {
            ...
            "error": {
                "__type": "No specifications",
                "name": [
                    "No specifications provided to create API token for an existing user in the Data Catalog. Please specify the username and a name for the new API token."
                ],
            },
        }
        return jsonify(response)
    ...
    # Make a POST request to the CKAN API with the parameters
    response = requests.post(
        config["CKAN_API"] + "api_token_create",
        json=token_metadata,
        headers=package_headers,
    )  # auth=HTTPBasicAuth(config.username, config.password))
...
@app.input(
    schema.Identifier, location="query", example="778bc28b-627c-472f-9d78-4d3617733218"
)
# @app.output(schema.ResponseOK, status_code=200)
@app.doc(tags=["User Management"])
def api_user_organization(query_data):
    ...
    if request.method == "GET" and "id" in query_data:
        ...
        response = requests.get(
            config["CKAN_API"] + "organization_list_for_user?id=" + id
...
def api_user_id(query_data):
    ...
    if "id" in query_data:
        ...
        response = {
            ...
            "error": {
                "__type": "No specifications",
                "name": [
                    "No identifier provided. Please specify the id or account name of the requested user."
                ],
            },
        }
        return jsonify(response)
    ...
    # IMPORTANT! CKAN requires NO authentication for GET requests
    response = requests.get(
        config["CKAN_API"] + "user_show?id=" + id
    )  # , headers=config.package_headers)  #auth=HTTPBasicAuth(config.username, config.password))
...
def api_vocabularies():
    ...
    # IMPORTANT! CKAN requires NO authentication for GET requests
    response = requests.get(
        config["CKAN_API"] + "vocabulary_list"
    )  # , headers=config.package_headers) # auth=HTTPBasicAuth(config.username, config.password))
...
def api_resource_id(query_data):
    ...
    # IMPORTANT! CKAN requires NO authentication for GET requests
    response = requests.get(
        config["CKAN_API"] + "resource_show?id=" + id
    )  # , headers=config.package_headers)  #auth=HTTPBasicAuth(config.username, config.password))
...
def api_resource_search(query_data):
    ...
    # IMPORTANT! CKAN requires NO authentication for GET requests
    response = requests.get(
        config["CKAN_API"] + "resource_search?query=" + q
    )  # , headers=config.package_headers)  # auth=HTTPBasicAuth(config.username, config.password))
...
@app.input(
    ...
    example={
        "package_metadata": {
            ...
            "extras": [
                {"key": "custom_tags", "value": "http://www.w3.org/ns/dcat#Dataset"},
                {"key": "INSPIRE theme", "value": "Location"},
                {"key": "Topic", "value": "POI"},
            ],
        ...
        if "package_metadata" in metadata:
            ...
            response = {
                ...
                "error": {
                    "__type": "No specifications",
                    "name": [
                        "No metadata provided for publishing in the Catalog. Please specify at least some basic metadata (title, notes, tags, etc.) for the dataset you wish to publish."
                    ],
                },
            }
            return jsonify(response)
        ...
        if "package_metadata" in metadata:
            ...
            response = {
                ...
                "error": {
                    "__type": "No specifications",
                    "name": [
                        "No metadata provided for updating this dataset in the Catalog. Please specify metadata for the dataset you wish to patch."
                    ],
                },
            }
            return jsonify(response)
...
def api_resource_upload(json_data):
    ...
    if request.data:
        ...
        if "resource_metadata" in metadata:
            ...
            response = {
                ...
                "error": {
                    "__type": "No specifications",
                    "name": [
                        "No metadata provided for updating this resource in the Catalog. Please specify metadata for the resource you wish to upload."
                    ],
                },
            }
            return jsonify(response)
        ...
        response = {
            ...
            "error": {
                "__type": "No specifications",
                "name": [
                    "No metadata provided for publishing this resource in the Catalog. Please specify metadata for the resource you wish to upload."
                ],
            },
        }
        return jsonify(response)
    ...
    response = {
        ...
        "error": {
            "__type": "Not found",
            "name": [
                "The specified file resource is not found or could not be accessed."
            ],
        },
    }
    return jsonify(response)
...
def api_workflow_publish(json_data):
    ...
    if request.data:
        ...
        if "package_metadata" in metadata:
            package_metadata = metadata["package_metadata"]
            # Also create the name of the new CKAN package from its title (assuming that this is unique)
            package_metadata["name"] = re.sub(
                r"[\W_]+", "_", package_metadata["title"]
            ).lower()
            # Convert the tags into the format required by CKAN
                ...
                if len(org_json) > 0:
                    for item in org_json:
                        if (
                            ...
                            package_metadata["owner_org"] = org_json[0][
                                "name"
                            ]  # CAUTION! Taking the first organization where this user is editor
                            break
        else:
            response = {
                ...
                "error": {
                    "__type": "No specifications",
                    "name": [
                        "No metadata provided for publishing in the Catalog. Please specify at least some basic metadata (title, notes, tags, etc.) for the workflow you wish to publish."
                    ],
                },
            }
            return jsonify(response)
    ...
    if specs.get("artifact_metadata") != None:
        ...
        return {
            ...
            "error": {
                "__type": "No specifications",
                "name": [
                    "No metadata provided for publishing this artifact in the Catalog. Please specify metadata for the artifact you wish to publish."
                ],
            },
        }
...
def api_dataset_purge(json_data):
    ...
    if request.data:
        ...
        response = {
            ...
            "error": {
                "__type": "No specifications",
                "name": [
                    "No metadata provided for deleting a dataset from the Catalog. Please specify the id of the dataset you wish to permanently delete."
                ],
            },
        }
        return jsonify(response)
    ...
    response = requests.post(
        config["CKAN_API"] + "dataset_purge",
        json=delete_metadata,
        headers=package_headers,
    )  # auth=HTTPBasicAuth(config.username, config.password))
    return response.json()
...
def api_dataset_unpublish(json_data):
    ...
    if request.data:
        ...
        response = {
            ...
            "error": {
                "__type": "No specifications",
                "name": [
                    "No metadata provided for unpublishing a dataset from the Catalog. Please specify the id of the dataset you wish to unpublish."
                ],
            },
        }
        return jsonify(response)
    ...
    response = requests.post(
        config["CKAN_API"] + "package_delete",
        json=unpublish_metadata,
        headers=package_headers,
    )  # auth=HTTPBasicAuth(config.username, config.password))
    return response.json()
...
@app.input(
    schema.Identifier,
    location="json",
    example={"id": "aa2992aa-b589-463d-ae1e-8430d91206cb"},
)
@app.output(schema.ResponseOK, status_code=200)
@app.doc(tags=["Catalog Management"])
@app.auth_required(auth)
def api_resource_delete(json_data):
    ...
    if request.data:
        ...
        response = {
            ...
            "error": {
                "__type": "No specifications",
                "name": [
                    "No metadata provided for deleting a resource from the Catalog. Please specify the id of the resource you wish to permanently delete."
                ],
            },
        }
        return jsonify(response)
    ...
    # Make a POST request to the CKAN API to purge an existing dataset
    response = requests.post(
        config["CKAN_API"] + "resource_delete",
        json=delete_metadata,
        headers=package_headers,
    )  # auth=HTTPBasicAuth(config.username, config.password))
    return response.json()
...
def api_task_execution_create(json_data):
    ...
    try:
        ...
        if state != "running":
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "This workflow no longer accepts tasks",
                    }
                ),
                500,
            )
        ...
        response = sql_utils.task_execution_create(
            task_exec_id, workflow_exec_id, start_date, state, tags
        )
        if not response:
            return (
                jsonify(
        ...
        tags["container_id"] = engine.create_task(
            docker_image, request.headers.get("Api-Token"), task_exec_id
        )
...
@app.input(
    schema.Identifier, location="query", example="24a976c4-fd84-47ef-92cc-5d5582bcaf41"
)
# @app.output(schema.ResponseOK, status_code=200)
@app.doc(tags=["Tracking Operations"])
@app.auth_required(auth)
            ...
            if path is None:
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": f"This resource {res_id} cannot be fetched by CKAN",
                        }
                    ),
                    500,
                )
...
def api_task_execution_output_json(json_data):
    ...
    try:
        ...
        if not response:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Task Execution could not be commited.",
                    }
                ),
                500,
            )
        ...
        #### INSERT LOG
        response = sql_utils.task_execution_insert_log(
            task_exec_id, output_json.get("message", "")
        )
        if not response:
            return (
                jsonify(
        ...
        for file in output_json["output"]:
            ...
            if response["success"]:
                ...
                return (
                    jsonify(
                        {"success": False, "message": "Error in publishing in CKAN"}
                    ),
                    500,
                )

        #### INSERT OUTPUT FILES
        response = sql_utils.task_execution_insert_output(
            task_exec_id, output_resource_ids
        )
        if not response:
            return (
                jsonify(
...
@app.input(
    schema.Identifier, location="query", example="4a142419-2342-4495-bfa3-9b4b3c2cad2a"
)
# @app.output(schema.ResponseOK, status_code=200)
@app.doc(tags=["Tracking Operations"])
@app.auth_required(auth)
    ...
    try:
        ...
        if not response:
            return (
                jsonify(
                    {
                        "success": True,
                        "message": f"The Task {task_exec_id} could not be deleted.",
                    }
                ),
                500,
            )
        return (
            jsonify(
                {
                    "success": True,
                    "message": f"The Task {task_exec_id} was deleted successfully",
                }
            ),
            200,
        )
...
def api_workflow_execution_create(json_data):
    ...
    try:
        ...
        response = sql_utils.workflow_execution_create(
            workflow_exec_id, start_date, state, tags
        )
        if not response:
            return (
                jsonify(
...
def api_workflow_execution_commit(json_data):
    ...
    try:
        ...
        response = sql_utils.workflow_execution_update(
            workflow_exec_id, state, end_date
        )
        if not response:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Workflow Execution could not be commited.",
                    }
                ),
                500,
            )
    ...
    try:
        ...
        if not response:
            return (
                jsonify(
                    {
                        "success": True,
                        "message": f"The Task {workflow_exec_id} could not be deleted.",
                    }
                ),
                500,
            )
        return (
            jsonify(
                {
                    "success": True,
                    "message": f"The Task {workflow_exec_id} was deleted successfully",
                }
            ),
            200,
        )
...
if __name__ == "__main__":
    ...
    app.config["AUTO_OPERATION_SUMMARY"] = app.config["settings"][
        "API_AUTO_OPERATION_SUMMARY"
    ]
    app.config["AUTO_OPERATION_DESCRIPTION"] = app.config["settings"][
        "API_AUTO_OPERATION_DESCRIPTION"
    ]
    ...
    if "execution" not in app.config["settings"] and "EXECUTION_ENGINE" in os.environ:
        ...
        app.config["settings"]["execution"] = {"engine": os.environ["EXECUTION_ENGINE"]}
    ...
    # Deploy the API
    app.run(
        host=app.config["settings"]["FLASK_RUN_HOST"],
