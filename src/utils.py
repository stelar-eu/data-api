...
identifiers_sql_filter_template = (
    "SELECT id, value, 0.0 AS score FROM _VIEW WHERE value IS NOT NULL _IDS"
)
...
sql_views = {
    "tags": "package_tag_array",
    "language": "package_language_array",
    "theme": "package_theme_array",
    "license": "package_license_array",
    "dataset_type": "package_dataset_type_array",
    "format": "package_format_array",
    "provider_name": "package_provider_array",
    "organization": "package_organization_array",
    "spatial": "package_extent",
    "temporal_extent": "package_temporal_extent",
    "metadata_modified": "package_metadata_modified",
    "num_rows": "package_num_rows",
    "days_active": "package_days_active",
    "velocity": "package_velocity",
    "cloud_coverage": "profile_vista_min_cloud_coverage",
    "missing": "profile_vista_min_missing",
    "lai": "profile_vista_max_lai",
}
...
sql_workflow_execution_templates = {
    ...
    "workflow_task_statuses_template": """SELECT t.state AS exec_state,
                                                COUNT(*) FROM 
                                                klms.workflow_execution p 
                                                JOIN klms.task_execution AS t 
                                                ON t.workflow_uuid = p.workflow_uuid 
                                                WHERE p.workflow_uuid=%s 
                                                GROUP BY t.state;
    """,
    ...
    "resource_lineage_with_depth_template": """
        WITH RECURSIVE lineage (
            ...
            JOIN klms.task_input ti ON to_tbl.task_uuid = ti.task_uuid
            JOIN lineage l ON to_tbl.dataset_id = l.input_resource_id
            WHERE (%s IS NULL OR l.depth + 1 <= %s)
        )
        ...
        ORDER BY l.output_task_uuid, l.input_order
    """,
    "resource_forward_lineage_with_depth_template": """WITH RECURSIVE forward_lineage (
        input_resource_id,
        input_task_uuid,
        input_order,
        input_group_name,
        ...
        -- Base case: start from resource being tracked
        SELECT
            ti.resource_id AS input_resource_id,
            ti.task_uuid AS input_task_uuid,
            ti.order_num AS input_order,
        ...
        -- Recursive case: find all downstream tasks/resources
        SELECT
            ti.resource_id,
            ti.task_uuid,
            ti.order_num,
            ...
            to_tbl.dataset_id,
            to_tbl.output_name,
            fwd.depth + 1 AS depth
        FROM klms.task_input ti
        JOIN klms.task_output to_tbl ON ti.task_uuid = to_tbl.task_uuid
        JOIN forward_lineage fwd ON ti.resource_id = fwd.output_resource_id
        WHERE (%s IS NULL OR fwd.depth + 1 <= %s)
    ),

    tasks_with_all_inputs AS (
        -- For every discovered task, expand all its inputs
        SELECT
            fwd.output_task_uuid AS task_uuid,
            ti.resource_id AS input_resource_id,
            ti.order_num AS input_order,
...
def handle_extras(json_metadata):
    ...
    for key, value in json_metadata.items():
        ...
        if key == "spatial":  # Special handling of GeoJSON or WKT for spatial extent
            item["value"] = validate_spatial(
                json_metadata["spatial"]
            )  # json.dumps(json_metadata['spatial'])
        elif isinstance(
            value, dict
        ):  # Convert a dictionary as required for extras in CKAN.
...
def assign_scores(
    ...
    for key, df in dict_df_facet_scores.items():
        if hasattr(df, "shape") and hasattr(df, "columns"):
            logger.debug(
                "Facet '%s' - DataFrame shape: %s, columns: %s",
                key,
                df.shape,
                list(df.columns),
            )
    ...
    for r in results:
        try:
            ...
            for attr in profile_attributes:
                try:
                    if (
                        attr in dict_df_facet_scores
                        and hasattr(dict_df_facet_scores[attr], "columns")
                        and "value" in dict_df_facet_scores[attr].columns
                        and hasattr(dict_df_facet_scores[attr], "index")
                        ...
                        logger.debug(
                            "Added profile for attr '%s': %s", attr, kv_pair["value"]
                        )
                    else:
                        logger.debug(
                            "Skipped attr '%s' - not found or invalid structure", attr
                        )
                except Exception as e:
                    logger.error(
                        "Error processing profile attribute '%s': %s", attr, str(e)
                    )
                    continue
            ...
            try:
                if (
                    hasattr(df_scores, "empty")
                    and not df_scores.empty
                    and hasattr(df_scores, "index")
                    and id in df_scores.index
                ):
                    r["score"] = float(df_scores.loc[id]["score"])
                    r["rank"] = int(df_scores.index.get_loc(id) + 1)
                    logger.debug(
                        "Assigned overall score: %s, rank: %s", r["score"], r["rank"]
                    )
                elif "score" in r:
                ...
                logger.error(
                    "Error assigning overall score for ID '%s': %s", id, str(e)
                )
                r["score"] = 0.0
            ...
            for key in dict_df_facet_scores.keys():
                try:
                    ...
                    if (
                        hasattr(facet_df, "empty")
                        and not facet_df.empty
                        and hasattr(facet_df, "index")
                        and id in facet_df.index
                    ):
                        ...
                        if hasattr(facet_df, "columns") and "score" in facet_df.columns:
                            ...
                            logger.debug(
                                "Found partial score for facet '%s': %s",
                                key,
                                partial_scores[key],
                            )
                        else:
                            logger.warning(
                                "No 'score' column in facet '%s' dataframe. Columns: %s",
                                key,
                                (
                                    list(facet_df.columns)
                                    if hasattr(facet_df, "columns")
                                    else "N/A"
                                ),
                            )
                        ...
                        logger.debug(
                            "ID '%s' not found in facet '%s' or dataframe is empty",
                            id,
                            key,
                        )
                        partial_scores[key] = 0.0
                    ...
                    logger.error(
                        "Error processing partial score for facet '%s', ID '%s': %s",
                        key,
                        id,
                        str(e),
                    )
            ...
            logger.error(
                "Error processing result %s: %s", r.get("id", "unknown"), str(e)
            )
...
def format_facet_preferences(json_metadata, sql_id_filter, k):
    ...
    for key in json_metadata.keys():
        if key in numerical_facets:  # NUMERICAL metadata elements
            if isinstance(
                json_metadata[key], list
            ):  # RANGE of numerical values acts as FILTER
                sql[key] = (
                    numerical_sql_range_template.replace("_VIEW", sql_views[key])
                    .replace("_ARGS", "array" + str(json_metadata[key]))
                    .replace("_IDS", sql_id_filter)
                )
            #                print('range', sql[key])
            else:  # SINGLE numerical value used for RANKING
                sql[key] = (
                    numerical_sql_rank_template.replace("_VIEW", sql_views[key])
                    .replace("_TOPK", str(k))
                    .replace("_ARGS", str(json_metadata[key]))
                    .replace("_IDS", sql_id_filter)
                )
        #                print('rank', sql[key])
        elif key in categorical_facets:  # CATEGORICAL metadata elements
            sql[key] = (
                categorical_sql_rank_template.replace("_VIEW", sql_views[key])
                .replace("_TOPK", str(k))
                .replace("_ARGS", "array" + str(json_metadata[key]))
                .replace("_IDS", sql_id_filter)
            )
            ...
            try:  # Examine several specs for geometries
                if isinstance(json_metadata[key], dict):  # GeoJSON as dictionary
                    g = (
                        "ST_GeomFromGeoJSON("
                        + "'{0}'".format(json.dumps(json_metadata[key]))
                        + ")"
                    )
                    ...
                    g = (
                        "ST_GeomFromGeoJSON("
                        + "'{0}'".format(json.loads(str(json_metadata[key])))
                        + ")"
                    )
                ...
                try:  # Then, assuming this is WKT in WGS84
                    g = (
                        "ST_GeomFromText("
                        + "'{0}'".format(shapely.wkt.loads(str(json_metadata[key])))
                        + ", 4326)"
                    )
                    geom = shapely.wkt.loads(str(json_metadata[key]))
                except:  # Empty polygon
                    g = "ST_GeomFromGeoJSON(" + "'{0}'".format(
                        '{"type":"Polygon","coordinates":[]}'
                    )
                    geom = GeometryCollection()
            if isinstance(geom, Polygon):  # spatial extent
                sql[key] = (
                    spatial_sql_rank_template.replace("_VIEW", sql_views[key])
                    .replace("_TOPK", str(k))
                    .replace("_ARGS", g)
                    .replace("_IDS", sql_id_filter)
                )
                ...
                sql[key] = (
                    location_sql_rank_template.replace("_VIEW", sql_views[key])
                    .replace("_TOPK", str(k))
                    .replace("_ARGS", g)
                    .replace("_IDS", sql_id_filter)
                )
            ...
            sql[key] = (
                temporal_sql_rank_template.replace("_VIEW", sql_views[key])
                .replace("_TOPK", str(k))
                .replace("_ARGS", "array" + str(json_metadata[key]))
                .replace("_IDS", sql_id_filter)
            )
            ...
            if isinstance(
                json_metadata[key], list
            ):  # RANGE of date/time values acts as FILTER
                sql[key] = (
                    recency_sql_range_template.replace("_VIEW", sql_views[key])
                    .replace("_ARGS", "array" + str(json_metadata[key]))
                    .replace("_IDS", sql_id_filter)
                )
            #                print('range', sql[key])
                ...
                sql[key] = (
                    recency_sql_rank_template.replace("_VIEW", sql_views[key])
                    .replace("_TOPK", str(k))
                    .replace("_ARGS", "'" + str(json_metadata[key]) + "'")
                    .replace("_IDS", sql_id_filter)
                )
...
def processRasterProfile(resource_id, prof, sql):
    ...
    for var in prof:  # ['variables']:
        ...
        ext_raster_tags.append(
            "date"
        )  # Include original 'date' temporarily in order to convert it to an interval
        ...
        raster_metadata["spatial_resolution"] = raster_metadata["spatial_resolution"][
            "pixel_size_x"
        ]
        # If a single date is reported, replace it with a time interval
        if (
            "date" in raster_metadata
            and not "start_date" in raster_metadata
            and not "end_date" in raster_metadata
        ):
            raster_metadata["start_date"] = datetime.strptime(
                raster_metadata["date"], "%d.%m.%Y"
            ).strftime("%Y-%m-%d")
        ...
        for band in var["bands"]:
            ...
            band_distribution = cleanupDict(
                copy.deepcopy(band), numerical_distribution_tags
            )
            band_distribution["distr_id"] = str(
                uuid.uuid1()
            )  # Generate a UUID for this distribution
            sql.append(
                prepareInsertSql(band_distribution, "klms.numerical_distribution")
            )
            ...
            attr_metadata["attr_id"] = band[
                "uuid"
            ]  # Reuse the UUID already included in the original profile
            ...
            band_metadata = {}
            band_metadata["raster_name"] = raster_metadata[
                "name"
            ]  # Name of the raster as included in the original profile
            band_metadata["attr_id"] = band[
                "uuid"
            ]  # Reuse the UUID already included in the original profile
            ...
            if "no_data_distribution" in band:
                ...
                class_distribution = cleanupListDict(
                    copy.deepcopy(band["no_data_distribution"]),
                    categorical_distribution_tags,
                )
...
def processHierarchicalProfile(resource_id, prof, sql):
    ...
    depth_distribution = cleanupDict(
        copy.deepcopy(prof["depth_distribution"]), numerical_distribution_tags
    )
    depth_distribution["distr_id"] = str(
        uuid.uuid1()
    )  # Generate a UUID for this distribution
...
def processRdfGraphProfile(resource_id, prof, sql):
    ...
    degree_centrality_distribution = cleanupDict(
        copy.deepcopy(prof["degree_centrality_distribution"]),
        numerical_distribution_tags,
    )
    ...
    degree_distribution = cleanupDict(
        copy.deepcopy(prof["degree_distribution"]), numerical_distribution_tags
    )
    degree_distribution["distr_id"] = str(
        uuid.uuid1()
    )  # Generate a UUID for this distribution
    ...
    in_degree_distribution = cleanupDict(
        copy.deepcopy(prof["in_degree_distribution"]), numerical_distribution_tags
    )
    in_degree_distribution["distr_id"] = str(
        uuid.uuid1()
    )  # Generate a UUID for this distribution
    ...
    out_degree_distribution = cleanupDict(
        copy.deepcopy(prof["out_degree_distribution"]), numerical_distribution_tags
    )
    out_degree_distribution["distr_id"] = str(
        uuid.uuid1()
    )  # Generate a UUID for this distribution
    ...
    if "class_distribution" in prof:
        class_uuid = str(uuid.uuid1())  # Generate a UUID for this distribution
        class_distribution = cleanupListDict(
            copy.deepcopy(prof["class_distribution"]), categorical_distribution_tags
        )
        for item in class_distribution:
            item["distr_id"] = class_uuid
            # Rename property as required by the schema
...
def processTextualProfile(resource_id, prof, sql):
    ...
    for var in prof:  # ['variables']:
        ...
        if "sentence_length_distribution" in var:
            sentence_length_distribution = cleanupDict(
                copy.deepcopy(var["sentence_length_distribution"]),
                numerical_distribution_tags,
            )
            sentence_length_distribution["distr_id"] = str(
                uuid.uuid1()
            )  # Generate a UUID for this distribution
            sql.append(
                prepareInsertSql(
        ...
        if "word_length_distribution" in var:
            ...
            word_length_distribution["distr_id"] = str(
                uuid.uuid1()
            )  # Generate a UUID for this distribution
            sql.append(
                prepareInsertSql(
                    word_length_distribution, "klms.numerical_distribution"
                )
            )
            text_metadata["word_length_distribution"] = word_length_distribution[
                "distr_id"
            ]
        if "language_distribution" in var:
            lang_uuid = str(uuid.uuid1())  # Generate a UUID for this distribution
            lang_distribution = cleanupListDict(
                copy.deepcopy(var["language_distribution"]), language_distribution_tags
            )
        ...
        if "special_characters_distribution" in var:
            ...
            chars_distribution = cleanupListDict(
                copy.deepcopy(var["special_characters_distribution"]),
                categorical_distribution_tags,
            )
...
def extractProfileProperties(resource_id, profile):
    ...
    prof = profile[
        "table"
    ]  # No need to generate a UUID for this dataset; it has already obtained one as a CKAN resource
    ...
    for var in profile["variables"]:
        ...
        if var["type"] == "Numeric":
            ...
            numerical_distribution = cleanupDict(
                copy.deepcopy(var), numerical_distribution_tags
            )
            #            numerical_distribution['attr_id'] = attr_uuid
            numerical_distribution["distr_id"] = str(
                uuid.uuid1()
            )  # Generate a UUID for this distribution
            sql.append(
                prepareInsertSql(numerical_distribution, "klms.numerical_distribution")
            )
            # Compose values concerning the numerical attribute
            numerical_attribute = {}
            numerical_attribute["attr_id"] = attr_uuid
            numerical_attribute["value_distribution"] = numerical_distribution[
                "distr_id"
            ]
            sql.append(
                prepareInsertSql(numerical_attribute, "klms.numerical_attribute")
            )
            ...
            frequency_distribution = cleanupListDict(
                copy.deepcopy(var["frequency_distribution"]),
                categorical_distribution_tags,
            )
            ...
            sql.append(
                prepareInsertSql(categorical_attribute, "klms.categorical_attribute")
            )
            ...
            if "num_chars_distribution" in var:
                chars_uuid = str(uuid.uuid1())  # Generate a UUID for this distribution
                chars_distribution = cleanupDict(
                    copy.deepcopy(var["num_chars_distribution"]),
                    numerical_distribution_tags,
                )
                ...
                sql.append(
                    prepareInsertSql(chars_distribution, "klms.numerical_distribution")
                )
            if "num_words_distribution" in var:
                words_uuid = str(uuid.uuid1())  # Generate a UUID for this distribution
                words_distribution = cleanupDict(
                    copy.deepcopy(var["num_words_distribution"]),
                    numerical_distribution_tags,
                )
                ...
                sql.append(
                    prepareInsertSql(words_distribution, "klms.numerical_distribution")
                )
            # Must have included foreign keys to the various distributions
            ...
            if "length_distribution" in var:
                ...
                length_distribution = cleanupDict(
                    copy.deepcopy(var["length_distribution"]),
                    numerical_distribution_tags,
                )
                ...
                sql.append(
                    prepareInsertSql(length_distribution, "klms.numerical_distribution")
                )
            if "area_distribution" in var:
                area_uuid = str(uuid.uuid1())  # Generate a UUID for this distribution
                area_distribution = cleanupDict(
                    copy.deepcopy(var["length_distribution"]),
                    numerical_distribution_tags,
                )
                ...
                sql.append(
                    prepareInsertSql(area_distribution, "klms.numerical_distribution")
                )
            if "geom_type_distribution" in var:
                geomtype_uuid = str(
                    uuid.uuid1()
                )  # Generate a UUID for this distribution
                geom_type_distribution = cleanupListDict(
                    copy.deepcopy(var["geom_type_distribution"]),
                    categorical_distribution_tags,
                )
...
def extractResourceProperties(resource_id, metadata):
    ...
    # Common handling of profiles for Tabular and TimeSeries data
    if (
        metadata["resource_type"] == "Tabular"
        or metadata["resource_type"] == "TimeSeries"
    ):
...
def prepareInsertSql(metadata, table):
    ...
    values = ", ".join(
        "'{0}'".format(str(item).replace("'", r"''")) for item in metadata.values()
    )
...
def prepareZenodoMetadata(dataset, creator, creator_org, doi: None):
    ...
    spatial = next(
        (item["value"] for item in dataset["extras"] if item["key"] == "spatial"), None
    )
    spatial_resolution_in_meters = next(
        (
            item["value"]
            for item in dataset["extras"]
            if item["key"] == "spatial_resolution_in_meters"
        ),
        None,
    )
    temporal_start = next(
        (
            item["value"]
            for item in dataset["extras"]
            if item["key"] == "temporal_start"
        ),
        None,
    )
    temporal_end = next(
        (item["value"] for item in dataset["extras"] if item["key"] == "temporal_end"),
        None,
    )
    frequency = next(
        (item["value"] for item in dataset["extras"] if item["key"] == "frequency"),
        None,
    )
    documentation = next(
        (item["value"] for item in dataset["extras"] if item["key"] == "documentation"),
        None,
    )
    language = next(
        (item["value"] for item in dataset["extras"] if item["key"] == "language"), None
    )
    theme = next(
        (item["value"] for item in dataset["extras"] if item["key"] == "theme"), None
    )
    alternate_identifier = next(
        (
            item["value"]
            for item in dataset["extras"]
            if item["key"] == "alternate_identifier"
        ),
        None,
    )
    ...
    if temporal_start or temporal_end:
        ...
        timespan = {
            "type": "Valid"
        }  # Assuming that timespan specified the time period when dataset is valid
        if temporal_start:
    ...
    zenodo_metadata = {
        ...
        "license": license,
        "doi": doi,
        "grants": [{"id": "10.13039/501100000780::101070122"}],
    }
