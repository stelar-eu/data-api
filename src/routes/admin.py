...
def handle_user(user_id):
    if request.method == "GET":
        ...
        try:
            ...
            if user.get("username") == "admin":
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "Modifications to the 'admin' account are not allowed.",
                        }
                    ),
                    403,
                )
            ...
            if username:
                users_with_same_username = keycloak_admin.get_users(
                    {"username": username}
                )
            ...
            # Handle role changes
            current_roles = keycloak_admin.get_realm_roles_of_user(
                user_id
                            ...
                            if role_info:
                                ...
                                return (
                                    jsonify(
                                        {
                                            "success": False,
                                            "message": f"Role '{role}' not found in realm roles",
                                        }
                                    ),
                                    404,
                                )
