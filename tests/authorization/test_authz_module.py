...
def fake_ckan_request(method, **kwargs):
    ...
    if method == "package_show":
        ...
        return {
            "id": kwargs.get("resource", "unknown"),
            "type": method.replace("_show", ""),
        }
...
def test_resource_permissions_type_create_permissions(monkeypatch):
    monkeypatch.setattr(
        authz_module.ku,
        "init_admin_client_with_credentials",
        fake_init_admin_client_with_credentials,
    )
    ...
    fake_admin.get_client_role = (
        lambda client_id, role_name: f"fake_client_role_{role_name}"
    )
    fake_admin.add_composite_realm_roles_to_role = lambda realm_role, roles: None
    monkeypatch.setattr(
        authz_module.ku, "init_admin_client_with_credentials", lambda: fake_admin
    )
...
@pytest.mark.skip(reason="This test is not yet implemented correctly.")
def test_parse_authz_config(monkeypatch, app_context):
    ...
    monkeypatch.setattr(
        authz_module.mon, "get_current_realm_roles", fake_get_current_realm_roles
    )
    monkeypatch.setattr(
        authz_module.mon, "get_current_policies", fake_get_current_policies
    )
    monkeypatch.setattr(
        authz_module.mon, "get_current_client_roles", fake_get_current_client_roles
    )
    monkeypatch.setattr(
        authz_module.rec, "update_roles_from_yaml", fake_update_roles_from_yaml
    )
    monkeypatch.setattr(
        authz_module.rec, "update_policies_from_yaml", fake_update_policies_from_yaml
    )
    monkeypatch.setattr(
        authz_module.rec, "update_client_roles", fake_update_client_roles
    )
    ...
    monkeypatch.setattr(
        authz_module.ku, "delete_client_roles", fake_delete_client_roles
    )
